...
if (($azione == 'mod' || $azione == 'canc') && $nid == '') {header('location:gest_nav.php');}
if(isset ($err))  echo $err;
// mostra stringa bottoni
switch ($azione)
{      case 'ins': 
       $bti = new bottoni_str('Voci di menu - scelta del tipo','nav');     
       $bti->bt_upd_ins_voce();  break; 
       
       case 'mod':  
       $btm = new bottoni_str('Voci di menu - modifica','nav');     
       $btm->bt_upd_mod();  break; 
       
       case 'canc' :
       $btc = new bottoni_str('Voci di menu - conferma cancellazione','nav'); 
       $btc->bt_upd_canc(); break;
...
case '':
    header('location:gest_nav.php');
      break;
case 'ins':    // scelta tipo voce, prosegue su: upd2_nav.php
    { 
      echo  "<fieldset >";
      $tmod = new DB_tip_i('voce','ntipo','','Tipo voce');     $tmod->select();
      echo  "</fieldset></form>";  
      break;
     }
...
{ 
    ...
    $row = mysql_fetch_array($_nav);
      ...
      $f1  = new field($nprog,'nprog',3,'Progressivo');             $f1->field_i();
      $ts  = new DB_tip_i('stato','nstat',$nstat,'Stato record');   $ts->select();
      $mnu = new DB_sel_l('mnu','bprog',$nmenu,'bmenu',
                          'nmenu','bstat','bmenu','Menu');  $mnu->select_label();
      $f2 = new field($nli,'nli',20,'Voce');                        $f2->field_i();
      $f4 = new field($ndesc,'ndesc',20,'Sottovoce');               $f4->field_i();      
      $f3 = new field($ntesto,'ntesto',25,'Descrizione');           $f3->field_i();
...
case 'ifr':
       {
       $tw = new select_file(DB::$path_site,$nsotvo,'nsotvo','Html/php pers.');    
       echo $tw->file(); 
       break;        
       } 
case 'htm':
       {$tw = new field($nsotvo,'nsotvo',30,'Pagina HTML custom');    echo $tw->field_i(); break;
       } 
case 'url':
      ...
      $tz = new field($naccesso,'naccesso',1,'Accesso');            $tz->field_i();      
      echo  "</fieldset></div>"; 
...
$tz = new field($naccesso,'naccesso',1,'Accesso');             $tz->field_r();
      echo  "</fieldset></div>";
