...
$azione    =$_POST['submit']; 
   ...
   default:
/** -------------------------------------------------------
Per il mese 01 si devono includere tutti i pagamenti 
eseguiti anche nell'anno precedente!   ?????  TOLTO
----------------------------------------------------------- */
$anno     = $_POST['anno'];
//$dd1      = new data($_POST['dadata']);
//$dadata   =$dd1->flipData($_POST['dadata']);
$date	  		=date_create($_POST['dadata']);
$dadata 	  		=date_format($date,"d/m/Y");
//$dd2      = new data($_POST['adata']); 
//$adata    =$dd2->flipData($_POST['adata']);
$date	  		=date_create($_POST['adata']);
$adata 	  		=date_format($date,"d/m/Y"); 
...
$pdf=new PDF('P','mm');
$pdf->Open();
$title=DB::$page_title." - VERSAMENTI MENSILI ".$_POST['anno']." dal ".$_POST['dadata']." al ".$_POST['adata'];
$pdf->AddPage();   
$tot = 0.00;
$pdf->SetFont('arial','',8);

// lettura dati
$sql = "SELECT * FROM `tbl_ver` as v , `tbl_isc` as e 
        WHERE (e.numero_iscrizione = v.vnume)
          and (v.vdata >= '$dadata')  
          and (v.vdata <= '$adata')
          and (v.vanno = '$anno')
        ORDER BY v.vdata";
