...
// lato destro                                                           
     ...
     $fy = new input(array('Italia','nascita_nazione',25,'Nazione di nascita','','i'));
          $fy->field();
     $fo = new input(array('00-00-0000','nascita_data',12,'Data di nascita','','d1'));    
          $fo->field();
     $fq = new input(array(date('d-m-Y'),'data_iscrizione',12,'Data iscrizione','','d2')); 
          $fq->field();
     $fz = new input(array('00-00-0000','data_uscita',12,'Data uscita','','d3')); 
          $fz->field();
...
// lato destro: Altri dati     
     ...
     $fm = new input(array($nascita_luogo,'nascita_luogo',25,'Luogo di nascita','','i'));  
          $fm->field();
     $fu = new input(array($prov_na,'prov_na',25,'Provincia di nascita','','i'));
          $fu->field(); 
     ...
     $fq = new input(array($data_iscrizione,'data_iscrizione',12,'Data iscrizione','','d2'));    
          $fq->field();
     $fz = new input(array($data_uscita,'data_uscita',12,'Data uscita','','d3'));    
          $fz->field();
...
// cancellazione ripristino mostra =======================================   
    case 'cancella' :
    case 'ripristina' :
    case 'mostra' :
    { $con = "mysql:host=".DB::$host.";dbname=".DB::$db."";
...
// lato destro: Altri dati                                                           
     ...
     $fq = new input(array($data_iscrizione,'data_iscrizione',12,'Data iscrizione','','r'));    
          $fq->field();
     $fq = new input(array($data_uscita,'data_uscita',12,'Data uscita','','r'));    
          $fq->field();
...
//  Incarichi e cariche
     ...
     $f3 = new input(array($cese,'icons_ese',25,'Comitato esecutivo','','r'));        
          $f3->field();
     $f3 = new input(array($volontario,'volontario',25,'Volontario','','r'));        
          $f3->field();
     echo  "</fieldset>";
     echo "</div>";   // col
     echo "</div>";  // row 
...
// lato destro: Altri dati                                                           
     ...
     $fq = new input(array($data_iscrizione,'data_iscrizione',12,'Data iscrizione','','r'));    
          $fq->field();
     $fq = new input(array($data_uscita,'data_uscita',12,'Data uscita','','i'));    
          $fq->field();
     $fs = new input(array($archiviare,'archiviare',2,'Coniuge','',r,));      
          $fs->field(); 
     $f1 = new DB_decxdb('prt',$stampa);
     ...
     $ft = new input(array($dec,'stampa',25,'Stampa','','r'));               
          $ft->field();
     $fu = new input(array($note,'note',33,'Note','','i')) ;                   
          $fu->field();
